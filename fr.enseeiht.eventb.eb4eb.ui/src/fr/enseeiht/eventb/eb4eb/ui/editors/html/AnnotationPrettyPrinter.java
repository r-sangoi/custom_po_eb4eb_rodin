...
public class AnnotationPrettyPrinter extends DefaultPrettyPrinter {
	...
	public void prettyPrint(IInternalElement elt, IInternalElement parent,
		...
		assert elt instanceof IAnnotation;
		StringBuffer availableTheoryProjectClause= new StringBuffer();
		try {
			IAnnotation availableTheory = (IAnnotation) elt;
			if(availableTheory.hasIdentifierString()){
				availableTheoryProjectClause.append("[");
				availableTheoryProjectClause.append(availableTheory.getIdentifierString());
				availableTheoryProjectClause.append("] ");
			}
